...
import { fetchCards, updateCard, deleteCard, addCard } from '../api/cardsApi';
import { Card } from '../api/types';

export const useCardsController = () => {
    const [cards, setCards] = useState<Card[]>([]);
    const [loading, setLoading] = useState(false);
    const [error, setError] = useState<string | null>(null);

    const fetchAllCards = async (params: { skip?: number; limit?: number; sortField?: string; sortOrder?: string } = {}) => {
        setLoading(true);
        try {
